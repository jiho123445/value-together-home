...
## 2026-09-13 추가 수정 — 연혁 저장 UI
- 관리자 > 기본정보/디자인 관리 > 연혁의 행별 저장 기능을 아이콘만 표시하지 않고 `저장` 버튼으로 명확하게 표시.
- 변경 사항이 있을 때 안내 문구를 표시하고, 저장 성공 시 `홈페이지 반영 완료`를 표시.
- 삭제 버튼도 `삭제` 텍스트를 함께 표시.
- 연혁 수정은 Firestore 저장 성공 후에만 관리자 상태에 반영되도록 변경하여 저장 실패 시 미저장 내용이 반영된 것처럼 보이지 않게 함.
